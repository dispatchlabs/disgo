...
type nodeIteratorState struct {
	hash    crypto.HashBytes // Hash of the node being iterated (nil if not standalone)
	node    node             // Trie node being iterated
	parent  crypto.HashBytes // Hash of the first full ancestor node (nil if current is the root)
...
func (it *nodeIterator) Hash() crypto.HashBytes {
	if len(it.stack) == 0 {
...
func (it *nodeIterator) Parent() crypto.HashBytes {
	if len(it.stack) == 0 {
		return crypto.HashBytes{}
	}
...
func (it *nodeIterator) nextChild(parent *nodeIteratorState, ancestor crypto.HashBytes) (*nodeIteratorState, []byte, bool) {
...
func (it *differenceIterator) Hash() crypto.HashBytes {
	return it.b.Hash()
}

func (it *differenceIterator) Parent() crypto.HashBytes {
	return it.b.Parent()
}
...
func (it *differenceIterator) Next(bool) bool {
	...
	for {
		...
		case 0:
			...
			hasHash := it.a.Hash() == crypto.HashBytes{}
...
func (it *unionIterator) Hash() crypto.HashBytes {
	return (*it.items)[0].Hash()
}

func (it *unionIterator) Parent() crypto.HashBytes {
...
func (it *unionIterator) Next(descend bool) bool {
	...
	for len(*it.items) > 0 && ((!descend && bytes.HasPrefix((*it.items)[0].Path(), least.Path())) || compareNodes(least, (*it.items)[0]) == 0) {
		...
		if skipped.Next(skipped.Hash() == crypto.HashBytes{}) {
			it.count++
