...
type headerMarshaling struct {
	...
	Time       *hexutil.Big
	Extra      hexutil.Bytes
	Hash       crypto.HashBytes `json:"hash"` // adds call to Hash() in MarshalJSON
}

// Hash returns the block hash of the header, which is simply the keccak256 hash of its
// RLP encoding.
func (h *Header) Hash() crypto.HashBytes {
	return rlpHash(h)
}

// HashNoNonce returns the hash which is used as input for the proof-of-work search.
func (h *Header) HashNoNonce() crypto.HashBytes {
...
func rlpHash(x interface{}) (h crypto.HashBytes) {
	hw := ethCrypto.NewKeccak256()
	rlp.Encode(hw, x)
	hw.Sum(h[:0])
	return h
}
...
func (b *Block) Transaction(hash crypto.HashBytes) *Transaction {
...
func (b *Block) ReceiptHash() crypto.HashBytes { return b.header.ReceiptHash }
func (b *Block) UncleHash() crypto.HashBytes   { return b.header.UncleHash }
...
func (b *Block) HashNoNonce() crypto.HashBytes {
...
func CalcUncleHash(uncles []*Header) crypto.HashBytes {
...
func (b *Block) Hash() crypto.HashBytes {
...
func DeriveSha(list DerivableList) crypto.HashBytes {
